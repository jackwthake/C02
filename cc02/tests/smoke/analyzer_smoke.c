...
int main(void) {
  ...
  assert(analyzer_insert_local(&a, "g", g) == 1);
  ...
  assert(analyzer_insert_local(&a, "p", p) == 1);
  ...
  assert(analyzer_insert_local(&a, "g", shadow) == 1); // allowed - different scope than outer "g"
  ...
  assert(analyzer_insert_local(&a, "p", dup) == 0);
