...
/*
 * Binary deserialization for ir_module_t (read side only).
 *
 * c02-as consumes IR object files; it never writes them. The writer lives
 * in the frontend that produces .o files - this file must stay layout-
 * compatible with it byte-for-byte.
 *
 * Every string in the IR (variable names, function names, field names,
 * struct_name inside type_t) is stored as [u32 length][bytes] and a fresh
 * copy is allocated from the arena on read. A null/empty string is a
 * length-0 record.
 *
 * Format: magic, version, then each section length-prefixed. Endianness
 * and integer widths are native (same-machine .o reuse only) - any writer
 * must emit native-endian, native-width to match.
 */
...
// Module lifecycle
// ----------------------------------------------------------------
// ir_read fills every array with exact-sized arena allocations, so init
// only has to stand up the arena and zero the module - a failed read then
// lands on a clean, freeable state.

int ir_gen_init(ir_gen_t *gen) {
  if (!arena_init(&gen->arena, IR_ARENA_CHUNK_SIZE))
    return 0;
  gen->module = (ir_module_t){0};
  return 1;
}
