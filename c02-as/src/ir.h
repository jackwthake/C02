...
// ----------------------------------------------------------------
// Type representation (self-contained)
// ----------------------------------------------------------------
// c02-as is codegen-only: it consumes a serialized IR object and never
// sees the AST, symbol table, or source. The handful of frontend types
// the IR carries by value are reproduced here so this header depends on
// nothing but the arena. These MUST stay layout-compatible with whatever
// writes the .o (see wire-format note on ir_read below).

typedef enum {
  TYPE_U8, TYPE_I8, TYPE_U16, TYPE_I16, TYPE_VOID, TYPE_STRUCT,
  TYPE_INVALID   // error sentinel
} type_kind_t;
...
typedef struct {
  ...
  unsigned is_ptr;
  unsigned ptr_depth;
  char *struct_name;    // only valid when kind == TYPE_STRUCT
} type_t;

// A single named, typed slot - a function parameter.
typedef struct {
  type_t  type;
  char   *name;
} param_t;
...
// Loaded-IR context
// ----------------------------------------------------------------
// Owns the deserialized module and the arena its strings/arrays live in.
// (Named ir_gen_t for continuity with the toolchain that produced it; in
// this codegen-only tool nothing is "generated" here - it is loaded.)
...
// Deserialize an IR object file into gen. The on-disk format is a frozen
// wire contract shared with the frontend that emits .o files: native
// endianness and native integer widths (same-machine .o reuse). Any writer
// (e.g. the Haskell frontend) must match ir-serial.c's read layout exactly.
