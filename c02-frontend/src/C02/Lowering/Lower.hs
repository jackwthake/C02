...
import C02.Analyzer.Types (Env (..), Symbol (VarSym), Ty, inferType)
import C02.Lowering.TAC (Instr (..), Operand (..), TacOp (..), binOpToTac, operandType)
import C02.Parser.AST (AssignmentOp (..), BaseType (..), BinOp (..), Block, Expr (..), Loc (Loc), Stmt (..), StructDecl (structFields, structName), UnOp (..), VarDecl (declInit, declName, ptrDepth, varType))
...
lowerStmt env _loop st (Assign lhs op rhs) =
  let valueExpr = case op of
      ...
      (storeInstrs, st2) = case lhs of
        ...
        Field base fname ->
          case base of
            -- Nested field assignment: o.in.v = 42
            -- Load outer base, load inner field into temp, store value into field of temp, store temp back
            Field outerBase outerFname ->
              let (outerBaseOp, outerBaseInstrs, st') = lowerExpr env st1 outerBase
                  (innerTemp, st'') = freshTemp st'
                  innerTempOp = OperandTemp (typeOf env base) innerTemp
                  loadInnerInstr = (emptyInstr TacFieldLoad) { instrDst = innerTempOp, instrSrc1 = outerBaseOp, instrFieldName = Just outerFname }
                  storeFieldInstr = (emptyInstr TacFieldStore) { instrDst = innerTempOp, instrSrc1 = val, instrFieldName = Just fname }
                  storeBackInstr = (emptyInstr TacFieldStore) { instrDst = outerBaseOp, instrSrc1 = innerTempOp, instrFieldName = Just outerFname }
               in (outerBaseInstrs ++ [loadInnerInstr, storeFieldInstr, storeBackInstr], st'')
            -- Shallow field assignment: x.v = 42 (current logic)
            _ ->
...
lowerStmt env loop st (For mInit mCond mIncr mBody) =
  let (env', initInstrs, st1) = lowerForInit env loop st mInit
   ...
   in ( initInstrs
          ++ labelInstr condLabel
          : condInstrs
          ++ bodyInstrs
          ++ labelInstr incrLabel
          : incrInstrs
...
lowerClauses env loop st _ [(Nothing, block)] =
  -- final `else`
  lowerBlock env loop st block -- no guard; falls through to end
lowerClauses env loop st end [(Just cond, block)] =
  -- final guarded clause, no else
...
lowerClauses env loop st end ((Just cond, block) : rest) =
  -- guarded clause, more follow
...
lowerClauses env loop st _ ((Nothing, block) : _) =
  -- else before end (AST forbids); take it
